export const sermonQueries = {
	...
	createSermon:
		`INSERT INTO SERMONS(title, speaker, date, series, reference, filepath) VALUES(?,?,?,?,?,?)`,
	updateSermon:
		`UPDATE cst391_milestone.sermons SET title=?, speaker=?, date=?, series=?, reference=?, filepath=? WHERE id = ?`,
	deleteSermon:
		`DELETE FROM SERMONS where id = ?`,
	readSpeakers:
		`SELECT DISTINCT speaker AS speaker FROM SERMONS`,
	readSeries:
		`SELECT DISTINCT series AS series FROM SERMONS`
}
