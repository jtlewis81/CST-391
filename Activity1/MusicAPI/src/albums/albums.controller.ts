...
export const readAlbums: RequestHandler = async (req: Request, res: Response) => {
    try {
        ...
        res.status(500).json({message: 'There was an error fetching albums'});
    }
}
...
export const readAlbumsByArtistSearch: RequestHandler = async (req: Request, res: Response) => {
    try {
        console.log('search', req.params.artist);
    ...
    try {
        ...
        req.body.tracks.forEach (async (track: Track, index: number) => {
            try {
                ...
                res.status(500).json({message: 'There was an error writing album tracks'});
            }
        });
        ...
        res.status(500).json({message: 'There was an error writing albums'});
    }
};
    ...
    try {
        ...
        req.body.tracks.forEach(async (track: Track, index: number) => {
            try {
                ...
                res.status(500).json({message:'There was an error updating album tracks'});
            }
        });
        ...
        console.error('[albums.controller][updateAlbum][Error] ', error);
        res.status(500).json({message:'There was an error updating albums'});
    }
};
...
export const deleteAlbum: RequestHandler = async (req: Request, res: Response) => {
    try {
        ...
        res.status(500).json({message:'There was an error deleting albums'});
    }
}
...
async function readTracks(albums: Album[], res: Response<any, Record<string, any>>) {
    for (let i = 0; i < albums.length; i++) {
        try {
            ...
            res.status(500).json({message: 'There was an error fetching album tracks'});
        }
    }
}
